...
pub fn do_send(ip: String, transmit_ip: String, stream_name: String, pa: PortAudio) {
    println!("(transmit mode) attempt to bind to {ip}");
    let socket = UdpSocket::bind(ip)
        .expect("couldn't bind to ip and port");

    println!("connecting to {transmit_ip}");
    socket.connect(transmit_ip)
    ...
    let stream: BlockingStream<i16> = create_stream(
        &pa,
    ...
    let max = 14.min(stream_name.len());
    for (index, c) in stream_name[0..max].bytes().enumerate() {
    ...
    loop {
        ...
        /* push the data */
        socket.send(&packet).expect("couldn't send audio packet");
    }
}
